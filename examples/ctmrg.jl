...
function peps_boundary(D_bond, D_bound)
    a,b,c,d = TensorMap(randn, D_bound, D_bound),TensorMap(randn, D_bound, D_bound),TensorMap(randn, D_bound, D_bound),TensorMap(randn, D_bound, D_bound)
    e,f,g,h = TensorMap(randn, D_bound*D_bond, D_bound),TensorMap(randn, D_bound*D_bond, D_bound),TensorMap(randn, D_bound*D_bond', D_bound),TensorMap(randn, D_bound*D_bond', D_bound)
...
function renorm_left!(boundary, Tensor; ϵ = 1e-2)
    ...
    isometry = tsvd(tmp; trunc=truncdim(ϵ))[1]
    ...
    boundary.C1 = C1/sqrt(norm(C1))
    boundary.E4 = E4/sqrt(norm(E4))
    boundary.C4 = C4/sqrt(norm(C4))
end
...
function optimize_boundary(Tensor, Tz,old_boundary=peps_boundary(ℂ^2,ℂ^3);numiter=100)

    for i in 1:numiter
        renorm_left!(old_boundary, Tensor; ϵ = 30)
    ...
    return calc_norm(old_boundary, Tz)./calc_norm(old_boundary, Tensor),old_boundary
end


βs = LinRange(0.0, 2., 100);
...
for (i,β) in enumerate(βs)
    ...
    (magns[i],env) = optimize_boundary(T,Tz,env,numiter=100);
...
plot(βs,abs.(magns),seriestype=:scatter,ylim=(-1,2))
